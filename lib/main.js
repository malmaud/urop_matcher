...
redisClient.on('ready', ()=>console.log(`Redis version: ${redisClient.server_info.redis_version}`));
...
function buildReact(req, page, props) {
  ...
  props.page = page;
  props.user = req.user;
  props.user_id = req.session ? req.session.user_id : null;
...
app.get('/login', (req, res)=>{
  let query = querystring.stringify({
    ...
    response_type: 'code',
    scope: 'openid email',
    redirect_uri: 'http://cocosci.me/google_redirect',
    state: '1'});
...
app.get('/google_redirect', async (req, res)=>{
  let data = {
    code: req.query.code,
    client_id: client_info.client_id,
  ...
  await session.associate_user_id(req, user_id);
  res.redirect('/');
});
...
app.route('/api/feedback')
  .post(async (req,res)=>{
    ...
    let data = {feedback: req.body.feedback};
    if(req.body.user_id) {
