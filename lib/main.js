...
async function open_db() {
  let c = await r.connect({host: '192.168.99.100', port: 28015, db: 'urop'});
  return c;
}
...
app.get('/', async (req, res)=> {
  let props = {};
  if(req.user.host_id && req.user.host_id.length > 0) {
...
app.route('/api/urop')
  .post(upload.single('cv'), async (req, res)=>{
    ...
    if(cv) {
      let filename = `${id}/${cv.originalname}`;
      let save_cv =  r.table('User').get(id).update({cv: filename}).run(req.conn);
...
app.route('/api/user/:user_id/cv')
  .get(async (req,res)=>{
    ...
    stream.on('end', async ()=>{
      ...
      let metadta = await file.getMetadataAsync();
