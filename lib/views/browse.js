...
var Browse = React.createClass({
  ...
  apply_filters() {
    ...
    var users = this.props.users.filter(user=>{
      let conditions = []
      conditions.push(user.kind == this.state.kind);
      if(this.state.show_unclaimed) {
        conditions.push(!user.host_id);
      }
    ...
    users.sort((a,b)=>{
      if (!a.approved) return -1;
        ...
        if(Date.parse(a.date_uploaded) < Date.parse(b.date_uploaded)) {
          return 1;
        } else {
          return -1;
        }
  ...
  onClaim(urop_id, should_claim) {
    let host_id;
    if(should_claim) {
      host_id = this.props.current_user.id;
    } else {
      host_id = '';
    }
    let payload = {host_id};
  ...
  render() {
    ...
    var profiles = users.map(user=> {
      let show_claim = true;
      return (<UserProfile  host={this.props.current_user}  onClaim={this.onClaim} onApprove={this.onApprove} user={user} key={user.user_id} show_claim={show_claim}/>);
    }
    );
    return (
      <div>
